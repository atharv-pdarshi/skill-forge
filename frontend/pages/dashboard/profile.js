...
const ProfilePage = () => {
  const { user, loading: authLoading } = useAuth(); // user from context might be slightly stale, profileData is fresher
  const [profileData, setProfileData] = useState(null);
  const [loadingProfile, setLoadingProfile] = useState(true);
  const [error, setError] = useState('');

  useEffect(() => {
    if (user && user.id) { // Check user.id to ensure user object is populated
      ...
      setLoadingProfile(false); // Not authenticated or user object not ready
    }
  }, [user, authLoading]);


  if (authLoading || (loadingProfile && !profileData)) { // Show loading if auth check or profile fetch is in progress
    return (
  ...
  const displayUser = profileData || user; // Prefer fresher profileData
