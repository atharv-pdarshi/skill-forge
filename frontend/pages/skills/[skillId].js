...
const SkillDetailPage = () => {
  ...
  const [loadingUserBooking, setLoadingUserBooking] = useState(false);
  ...
  const fetchUserBookingForSkill = useCallback(async () => {
    if (!isAuthenticated || !user || !skillId) {
        ...
        setLoadingUserBooking(false); // Ensure loading is false if conditions not met
        return;
    }
  ...
  useEffect(() => {
    if (skillId) {
      fetchSkillDetails();
      fetchSkillReviews();
      // Fetch user's booking for this skill if authenticated
      // This will run when skillId, isAuthenticated, or user changes.
      if (isAuthenticated && user?.id) { // Check for user.id as well
        fetchUserBookingForSkill();
      } else {
        setUserBookingForThisSkill(null); // Clear if user logs out or isn't fully loaded
        setLoadingUserBooking(false); // Ensure loading is stopped
      }
    }
  }, [skillId, fetchSkillDetails, fetchSkillReviews, isAuthenticated, user, fetchUserBookingForSkill]);
  ...
  const handleReviewSubmit = async (e) => {
    ...
    try {
      ...
      fetchSkillReviews(); // Re-fetch reviews to show the new one
    } catch (err) {
                ...
                <Card className="shadow-sm h-100" style={{ backgroundColor: 'var(--bg-secondary-dark)', borderColor: 'var(--border-color-dark)' }}>
                    ...
                    <Card.Body className="p-4 d-flex flex-column">
                        ...
                        <div className="d-flex flex-wrap gap-2 align-items-center mt-auto">
                            {/* Booking Button / Status Display Logic */}
                            ...
                            {isAuthenticated && !isProvider && !loadingUserBooking && userBookingForThisSkill && (
                                <Alert 
                                    variant={userBookingForThisSkill.status === 'pending' ? 'info' : getStatusBadgeVariant(userBookingForThisSkill.status)} 
                                    className="py-2 px-3 mb-0 w-100 text-center"
                                >
                                    ...
                                    {userBookingForThisSkill.status === 'confirmed' && 
                                        <Link href="/dashboard/bookings/student" passHref>
                                            <Button variant="link" size="sm" className="p-0 ms-2 align-baseline" style={{color: 'var(--accent-color)'}}>View Booking</Button>
                                        </Link>
                                    }
                                </Alert>
                            )}
                            {canBook && ( // This condition now correctly checks !userBookingForThisSkill && !loadingUserBooking
